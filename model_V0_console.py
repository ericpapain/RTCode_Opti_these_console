...
#Ratio Matieres premiere par produit ###ratio_pro(p,r)
...
#Capacite Matiere premiere par fournisseurs ###capa(f,r)
...
#Selection de fournisseurs  F(f)
F_CHOOSE = {f: model.addVar(vtype=GRB.BINARY, name=f"F_CHOOSE{f}") for f in FOURNISSEURS}

# Quantite de MP a commander chez chaque fournisseurs ####Q_MP(f,r)
Q_MP = {(f,r) : model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"Q_MP{f}_{r}") for f in FOURNISSEURS for r in MATIERES_PREMIERES}

# Quantite a produire par site   ####Q_PROD(u,p)
Q_PROP = {(u,p) : model.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"Q_PROD{u}_{p}") for u in USINES for p in PRODUITS}
